...
void mem_init(int framesize, int var_size)
{

	memory_size = frame_size + var_size;
...
/**
 * Loads up to the next three instructions for the process
 * into the frame store. Updates the page table of the process with
 * the location of the corresponding frames. If there is no place in
 * the frame store then we return 21
 */
int load_page(FILE *fp, PCB *process)
{
	...
	// shell memory is full, no frame available
	if (!hasSpaceLeft)
	{
		error_code = 21;
		return error_code;
	...
	int page_number = process->PC;
	int length = process->len;

	for (int k = process->PC; k < length; k++)
	{
		if (process->pageTable[page_number] != -1)
		{
			page_number++;
		}
	}
	...
	for (size_t j = i; j < i + 3 && j < frame_size; j++)
	{
		if (feof(fp) || page_number >= process->len)
		{
			break;
		}
		else
		{
			...
			sprintf(var, "%d", (int)j);
			shellmemory[j].var = strdup(var);

			process->pageTable[page_number++] = (int)j;
			shellmemory[j].value = strndup(line, strlen(line));
