...
int exec(char *fname1, char *fname2, char *fname3, char *policy, bool background, bool mt)
{
	...
	// updated implementation for exec can take duplicate files
	// if(fname2!=NULL){
	...
	if (fname1 != NULL)
	{
		error_code = store_initialize(fname1);
		if (error_code != 0)
		{
	...
	if (fname2 != NULL)
	{
		error_code = store_initialize(fname2);
		if (error_code != 0)
		{
			return handleError(error_code);
		}
	...
	if (fname3 != NULL)
	{
		error_code = store_initialize(fname3);
		if (error_code != 0)
		{
			return handleError(error_code);
		}
